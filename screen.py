...
class Screen:
    ...
    def register_key(
        self, name: str, key: int, mods: int, action: Callable[..., None],
        *args, **kwargs
    ) -> None:
        combo: tuple[int, int] = (key, mods)
        if combo in self.__active_keys:
            raise KeyError("Keypress action with keypresses" + combo +
                           "already exists")
    ...
    @property
    def active_keys(self) -> dict[tuple[int, int], tuple[str, Callable[..., None], dict[str, Any]]]:
    ...
    def register_click_listener(self, target: surf_rect.Surf_Rect, name: str, action: Callable[..., None], *args, **kwargs) -> None:
        ...
        self.__clickables[target] = (name, action, {"args": args, "kwargs": kwargs})
    ...
    def deregister_click_listener(self, target: surf_rect.Surf_Rect, name: str) -> None:
        ...
        if self.__clickables[target][0] != name:
            raise KeyError(f"Click listener has already been registered, albeit with a separate name. Please use {self.__clickables[target][0]} as the name to deregister it.")

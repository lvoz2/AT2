...
class Warrior(player.Player):
    def __init__(
        ...
        super().__init__(
            window.get_asset("assets/warrior.png"),
            name,
            "warrior",
            mask=mask,
...
# For simplicity, let's assume the warrior's damage is directly proportional
# to their level
...
#        target.take_damage(self.strength)  # Example: Charge deals damage equal to the
# warrior's strength
#
#    def basic_attack(self, target):
#        damage = self.strength  # Example: Basic attack damage equals warriors strength
...
#            damage = self.strength * 2  # Example: Cleave attack deals double the
# warrior's strength to each target
#            total_damage += damage
...
#        damage = self.strength + 5  # Example: Shield bash deals warrior's strength
# plus 5 additional damage
